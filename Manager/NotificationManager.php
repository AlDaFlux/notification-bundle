...
namespace Aldaflux\NotificationBundle\Manager;

use Doctrine\Common\Util\ClassUtils;
use Doctrine\ORM\EntityNotFoundException;
use Aldaflux\NotificationBundle\Entity\NotifiableEntity;
use Aldaflux\NotificationBundle\Entity\NotifiableNotification;
use Aldaflux\NotificationBundle\Entity\Notification;
use Aldaflux\NotificationBundle\Entity\NotificationInterface;
use Aldaflux\NotificationBundle\Event\NotificationEvent;
use Aldaflux\NotificationBundle\AldafluxNotificationEvents;
use Aldaflux\NotificationBundle\NotifiableDiscovery;
 ...
 * @package Aldaflux\NotificationBundle\Manager
 */
class NotificationManager
{
    protected $container;
    ...
    public function __construct(Container $container, NotifiableDiscovery $discovery)
    {
        ...
        $this->notifiableRepository = $this->om->getRepository('AldafluxNotificationBundle:NotifiableEntity');
        $this->notificationRepository = $this->om->getRepository('AldafluxNotificationBundle:Notification');
        $this->notifiableNotificationRepository = $this->om->getRepository('AldafluxNotificationBundle:NotifiableNotification');
    }
    ...
    {
        $notificationClass = $this->container->getParameter('aldaflux_notification.notification_class');
        ...
        $this->dispatcher->dispatch(AldafluxNotificationEvents::CREATED, $event);
    ...
    public function addNotification($notifiables, NotificationInterface $notification, $flush = false)
    {
        foreach ($notifiables as $notifiable) {
            ...
            $this->dispatcher->dispatch($event,AldafluxNotificationEvents::ASSIGNED);
        }
    ...
    {
        $repo = $this->om->getRepository('AldafluxNotificationBundle:NotifiableNotification');
        foreach ($notifiables as $notifiable) {
            ...
            $event = new NotificationEvent($notification, $notifiable);
            $this->dispatcher->dispatch(AldafluxNotificationEvents::REMOVED, $event);
            $this->flush($flush);
        }
    ...
    public function deleteNotification(NotificationInterface $notification, $flush = false)
    {
        ...
        $event = new NotificationEvent($notification);
        $this->dispatcher->dispatch(AldafluxNotificationEvents::DELETED, $event);
    }
    ...
    public function markAsSeen(NotifiableInterface $notifiable, NotificationInterface $notification, $flush = false)
    {
        ...
        if ($nn) {
            $nn->setSeen(true);
            $event = new NotificationEvent($notification, $notifiable);
            $this->dispatcher->dispatch(AldafluxNotificationEvents::SEEN, $event);
            $this->flush($flush);
        } else {
    ...
    public function markAsUnseen(NotifiableInterface $notifiable, NotificationInterface $notification, $flush = false)
    {
        ...
        if ($nn) {
            $nn->setSeen(false);
            $event = new NotificationEvent($notification, $notifiable);
            $this->dispatcher->dispatch(AldafluxNotificationEvents::UNSEEN, $event);
            $this->flush($flush);
        } else {
            throw new EntityNotFoundException('The link between the notifiable and the notification has not been found');
        }
    ...
    public function setDate(NotificationInterface $notification, \DateTime $dateTime, $flush = false)
    {
        ...
        $event = new NotificationEvent($notification);
        $this->dispatcher->dispatch(AldafluxNotificationEvents::MODIFIED, $event);
